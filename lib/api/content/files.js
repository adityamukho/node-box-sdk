...
module.exports = function (Connection) {
  Connection.addInstanceMethods(
    /** @lends Connection.prototype */
    {
       ...
       */
      getFile: function (id, version, dest, done) {
        if (!_.isNumber(parseInt(id, 10))) {
          return done(new Error('id must be specified.'));
        }
       ...
       * @param {requestCallback} done - The callback to invoke (with possible errors) when the request returns.
       */
      uploadFile: function (name, parent_id, opts, done) {
        ...
        this._request(['files', 'content'], 'post', done, null, opts, data);
      },
       ...
       * @param {RequestHeaders} [headers] - Additional headers.
       */
      deleteFile: function (id, done, headers) {
        if (!_.isNumber(parseInt(id, 10))) {
          return done(new Error('id must be specified.'));
        }

        this._request(['files', id], 'del', done, null, null, null, headers);
      },
       ...
       * @param {string} name - The fully qualified path to the local file.
       * @param {number} id - The file's ID.
       * @param {OptsUploadFileNewVersion} opts - Request options. Can be null.
       * @param {requestCallback} done - The callback to invoke (with possible errors) when the request returns.
       * @param {RequestHeaders} [headers] - Additional headers.
       */
      uploadFileNewVersion: function (name, id, opts, done, headers) {
        if (!_.isString(name) || !_.isNumber(parseInt(id, 10))) {
          return done(new Error('Invalid params. Required - name: string, id: number'));
        }

        var data = fs.createReadStream(name);
        this._request(['files', id, 'content'], 'post', done, null, opts, data, headers);
      },
       ...
       * @param {requestCallback} done - The callback to invoke (with possible errors) when the request returns.
       */
      getFileVersions: function (id, done) {
        if (!_.isNumber(parseInt(id, 10))) {
          return done(new Error('id must be specified.'));
        }

        this._request(['files', id, 'versions'], 'get', done);
      },
       ...
       * @param {requestCallback} done - The callback to invoke (with possible errors) when the request returns.
       */
      promoteFileVersion: function (id, version, done) {
        if (!_.isNumber(parseInt(id, 10))) {
          return done(new Error('id must be specified.'));
        }
        if (!_.isEmpty(version) && !_.isNumber(parseInt(version, 10))) {
        ...
        this._request(['files', id, 'versions', 'current'], 'post', done, null, payload);
      },
       ...
       * @param {RequestHeaders} [headers] - Additional headers.
       */
      deleteFileVersion: function (id, version, done, headers) {
        if (!_.isNumber(parseInt(id, 10))) {
          return done(new Error('id must be specified.'));
        }
        if (!_.isEmpty(version) && !_.isNumber(parseInt(version, 10))) {
        ...
        this._request(['files', id, 'versions', version], 'del', done, null, null, null, headers);
      },
       ...
       * @param {requestCallback} done - The callback to invoke (with possible errors) when the request returns.
       */
      copyFile: function (id, parent_id, name, done, headers) {
        ...
        this._request(['files', id, 'copy'], 'post', done, null, opts);
      },
       ...
       * @param {getFileCallback} done - The callback to invoke (with possible errors) when the request returns.
       */
      getFileThumbnail: function (id, opts, dest, done) {
        if (!_.isNumber(parseInt(id, 10))) {
          return done(new Error('id must be specified.'));
        }
        if (!_.isString(dest)) {
       ...
       * @param {requestCallback} done - The callback to invoke (with possible errors) when the request returns.
       */
      getTrashedFile: function (id, done) {
        if (!_.isNumber(parseInt(id, 10))) {
          return done(new Error('id must be specified.'));
        }
        this._request(['files', id, 'trash'], 'get', done);
      },
       ...
       * @param {requestCallback} done - The callback to invoke (with possible errors) when the request returns.
       */
      deleteTrashedFile: function (id, done) {
        if (!_.isNumber(parseInt(id, 10))) {
          return done(new Error('id must be specified.'));
        }
        this._request(['files', id, 'trash'], 'del', done);
      },
       ...
       * @param {number} id - The file's ID.
       * @param {string} name - The file's name. Can be null.
       * @param {number} parent_id - The parent folder's ID. Can be null.
       * @param {requestCallback} done - The callback to invoke (with possible errors) when the request returns.
       */
      restoreTrashedFile: function (id, name, parent_id, done) {
        ...
        this._request(['files', id], 'post', done, null, opts);
      },
       ...
       * @param {requestCallback} done - The callback to invoke (with possible errors) when the request returns.
       */
      getFileComments: function (id, done) {
        if (!_.isNumber(parseInt(id, 10))) {
          return done(new Error('id must be specified.'));
        }

        this._request(['files', id, 'comments'], 'get', done);
      },
       ...
       * @param {requestCallback} done - The callback to invoke (with possible errors) when the request returns.
       */
      getFileTasks: function (id, done) {
        if (!_.isNumber(parseInt(id, 10))) {
          return done(new Error('id must be specified.'));
        }

        this._request(['files', id, 'tasks'], 'get', done);
      }
    });
};